...
export async function batchProcessSKUs(skuList, storeInfo) {
  ...
  console.log(`===============================================`)
  // 使用与原始请求完全相同的URL格式
  // 从店铺信息中获取spShopNo，优先使用店铺信息中的spShopNo
  const spShopNo = storeInfo['店铺信息'] && storeInfo['店铺信息'].spShopNo
    ? storeInfo['店铺信息'].spShopNo
    : (storeInfo.spShopNo || storeInfo.shopNo || '18661988') // 默认值用于测试
...
/**
 * 根据SKU列表获取CSG编号列表 取消京配搜索
 * @param {Array<string>} skuList - SKU列表
 * @param {Object} deptInfo - 事业部信息
 * @returns {Promise<Array<string>>} CSG编号列表
 */
export async function getCSGList_cancelJpSearch(skuList) {
  if (!skuList || skuList.length === 0) {
    return { success: false, message: '未提供SKU列表', csgList: [] }
  }
  ...
  try {
    ...
    while (hasMoreData) {
      ...
      console.log('response_cancelJpSearch', response)
      // 处理响应
      if (response && response.aaData) {
        // 首次获取总记录数
        if (totalRecords === null) {
